                        ...
                        case NET_MESSAGE_GAME_PACKET: {
                            ...
                            switch (packet->m_type) {
                                ...
                                case PACKET_MODIFY_ITEM_INVENTORY: {
                                    int index = 0;
                                    //cout << "!! PACKET_MODIFY_ITEM_INVENTORY !!\n";
                                    string awebwje = to_string(packet->m_int_data) + "\n" + to_string(packet->m_jump_amount) + "\n" + to_string(packet->m_particle_time);
                                    //cout << awebwje;

                                    // EXPLANATION
                                    // packet->m_int_data = ITEMID
                                    // packet->m_jump_amount = AMOUNT
                                    /*for (Item& item : inventory) {
                                        if (item.id == packet->m_int_data) {
                                            server::inventory[index].id = packet->m_int_data;
                                            server::inventory[index].count = packet->m_jump_amount;
                                        }
                                    }*/
                                }
                                case PACKET_ITEM_CHANGE_OBJECT: {
                                    int index = 0;
                                    //cout << "!! PACKET_ITEM_CHANGE_OBJECT !!\n";
                                    string awebwje = to_string(packet->m_int_data) + "\n" + to_string(packet->m_jump_amount) + "\n" + to_string(packet->m_particle_time);
                                    //cout << awebwje;
                                ...
                                case PACKET_SEND_TILE_UPDATE_DATA_MULTIPLE:
                                    /*events::in::writetofile(packet);
                                    events::in::writetofile2(to_string(packet->m_data));
                                    events::in::writetofile2(to_string(packet->m_vec_x));
                                    events::in::writetofile2(to_string(packet->m_vec_y));
                                    events::in::writetofile2(to_string(packet->m_vec2_x));
                                    events::in::writetofile2(to_string(packet->m_vec2_y));
                                    events::in::writetofile2(to_string(packet->m_count));
                                    events::in::writetofile2(to_string(packet->m_data_size));
                                    events::in::writetofile2(to_string(packet->m_int_data));
                                    events::in::writetofile2(to_string(packet->m_item));
                                    events::in::writetofile2(to_string(packet->m_jump_amount));
                                    events::in::writetofile2(to_string(packet->m_packet_flags));
                                    events::in::writetofile2(to_string(packet->m_particle_time));
                                    events::in::writetofile2(to_string(packet->m_player_flags));
                                    events::in::writetofile2(to_string(packet->m_state1));
                                    events::in::writetofile2(to_string(packet->m_state2));
                                    events::in::writetofile2(to_string(packet->m_struct_flags));
                                    events::in::writetofile2(to_string(packet->m_type));*/
                                    break;
